...
class MainViewModel @Inject constructor(
    ...
    init {
        ...
        val lastSync = Realm.getDefaultInstance().use {
            realm -> realm.where(SyncLog::class.java)?.max("date") ?: 0
        }
        if (lastSync == 0 &&
    ...
    override fun bindView(view: MainView) {
        ...
        view.activityResumedIntent
            ...
            .distinctUntilChanged()
            .doOnNext { newState { copy(defaultSms = it) } }
            .autoDisposable(view.scope())
            .subscribe()
        ...
        view.activityResumedIntent
            ...
            .distinctUntilChanged()
            .doOnNext { newState { copy(smsPermission = it) } }
            .autoDisposable(view.scope())
            .subscribe()
        ...
        view.activityResumedIntent
            ...
            .distinctUntilChanged()
            .doOnNext { newState { copy(contactPermission = it) } }
            .autoDisposable(view.scope())
            .subscribe()
        ...
        view.activityResumedIntent
            ...
            .distinctUntilChanged()
            .doOnNext { newState { copy(notificationPermission = it) } }
            .autoDisposable(view.scope())
            .subscribe()
        ...
        view.onNewIntentIntent
            .autoDisposable(view.scope())
            .subscribe {
                when (it.getStringExtra("screen")) {
                    "compose" -> navigator.showConversation(it.getLongExtra("threadId", 0))
        ...
        view.queryChangedIntent
            ...
            .distinctUntilChanged()
            .doOnNext {
                newState { copy(page = ((page as? Searching) ?: Searching()).copy(loading = true)) }
            }
            .observeOn(Schedulers.io())
            .map(conversationRepo::searchConversations)
            .autoDisposable(view.scope())
            .subscribe { newState { copy(page = Searching(loading = false, data = it)) } }
...
//                        NavItem.HELP -> navigator.showSupport()
        ...
        view.optionsItemIntent
            .filter { it == R.id.archive }
            .withLatestFrom(view.conversationsSelectedIntent) { _, conversations ->
                markArchived.execute(conversations)
                conversations?.let {
                    lastArchivedThreadIds = it.toList()  // copy of selected convos
                    view.showArchivedSnackbar(it.count())
                }
                view.clearSelection()
            }
            .autoDisposable(view.scope())
        ...
        view.optionsItemIntent
            ...
            .filter { it.size == 1 }
            .mapNotNull {
                conversationRepo.getConversation(it.first())?.recipients?.firstOrNull()?.address
            }
            .doOnNext {
        ...
        view.optionsItemIntent
            .filter { it == R.id.rename }
            .withLatestFrom(view.conversationsSelectedIntent) { _, conversations ->
                conversations
            }
            .mapNotNull { it.firstOrNull()?.let { conversationRepo.getConversation(it) } }
            .autoDisposable(view.scope())
            .subscribe { view.showRenameDialog(it.name) }
...
//                }
        ...
        view.conversationsSelectedIntent
            .withLatestFrom(state) { selection, state ->
                ...
                when (state.page) {
                    is Inbox -> {
                        newState { copy(page = state.page.copy(
                            addContact = add,
                            markPinned = pin,
                            markRead = read,
                            selected = selected
        ...
        view.renameConversationIntent
            .withLatestFrom(view.conversationsSelectedIntent) { newName, conversations ->
                Pair(newName, conversations.firstOrNull())
            }
            .flatMapCompletable { (newName, conversationId) ->
                conversationId?.let {
                    conversationRepo.setConversationName(it, newName)
                        .subscribeOn(Schedulers.io())
            ...
            .subscribe { (threadId, direction) ->
                when (
                    ...
                    Preferences.SWIPE_ACTION_CALL ->
                        messageRepo.getMessagesSync(threadId).let { messages ->
                            // most recent non-me msg address
                            messages.lastOrNull { !it.isMe() }?.address
                            // or, if no 'not me' messages in convo, first recipient in convo
